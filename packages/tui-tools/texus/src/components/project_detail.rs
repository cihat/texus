...
impl ProjectDetail {
  pub fn draw(state: &AppState, frame: &mut Frame, area: Rect) {
    if let Some(project) = state.get_selected_project() {
      ...
      let mut scrollbar_state = ScrollbarState::new(details_text.lines().count()).position(state.detail_scroll);
  ...
  fn build_block<'a>() -> Block<'a> {
    ...
    let kill = Line::from("| Kill: k").style(Style::default().fg(Color::Red).bold());
